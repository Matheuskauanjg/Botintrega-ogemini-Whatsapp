...
export async function startMcpGateway({ publicPort, internalPort }) {
  ...
  async function internalJson(pathname, options = {}) {
    if (!API_TOKEN) {
      throw new Error('API_TOKEN is not configured on Render.');
    }
  ...
  function authMeta(scopes) {
    const schemes = [{ type: 'oauth2', scopes }];
    return { schemes, meta: { securitySchemes: schemes } };
  }

  function createWhatsappMcpServer() {
    const server = new McpServer({ name: 'meu-whatsapp', version: '1.1.0' });

    {
      const { schemes, meta } = authMeta(['whatsapp.read']);
...

    {
      ...
      server.registerTool('read_whatsapp_messages', {
        ...
        try { return result(await internalJson(`/api/chats/${encodeURIComponent(chatId)}/messages?limit=${encodeURIComponent(limit)}`)); }
        catch (error) { return errorResult(error.message); }
      });
    }
    ...
    {
      const { schemes, meta } = authMeta(['whatsapp.send']);
...
<style>body{font-family:Arial,sans-serif;background:#eef3f1;margin:0;min-height:100vh;display:grid;place-items:center;padding:24px}.card{max-width:460px;width:100%;background:white;padding:30px;border-radius:20px;box-shadow:0 10px 35px #0001}h1{margin-top:0}input{box-sizing:border-box;width:100%;padding:12px;margin:8px 0 16px;border:1px solid #ccd8d2;border-radius:10px;font-size:16px}button{width:100%;padding:12px;border:0;border-radius:10px;background:#1f8f55;color:white;font-size:16px;font-weight:700}.muted{color:#66766f;font-size:14px;line-height:1.5}</style>
</head><body><main class="card"><h1>Conectar Meu WhatsApp</h1><p>Autorize o ChatGPT a acessar o seu WhatsApp pessoal.</p><form method="post" action="/oauth/authorize">${hiddenInputs}<label>Chave privada</label><input type="password" name="access_key" autocomplete="current-password" required><button type="submit">Autorizar ChatGPT</button></form><p class="muted">Digite o valor de MCP_LOGIN_SECRET configurado no Render.</p></main></body></html>`);
  });

  app.post('/oauth/authorize', express.urlencoded({ extended: false, limit: '64kb' }), (req, res) => {
    if (!LOGIN_SECRET) {
      return res.status(503).type('html').send('<h1>Autenticação não configurada</h1>');
    }
  ...
  const parseTokenForm = express.urlencoded({ extended: false, limit: '64kb' });
  const parseTokenJson = express.json({ limit: '64kb' });
  app.post('/oauth/token', parseTokenForm, parseTokenJson, (req, res) => {
    const body = req.body || {};
  ...
  function requireMcpOAuth(req, res, next) {
    ...
    if (!payload) {
      console.log(`[MCP] ${req.method} /mcp -> 401 OAuth challenge`);
      res.set('WWW-Authenticate', challenge);
      return res.status(401).json({ error: 'unauthorized' });
    }

    req.mcpAuth = payload;
    console.log(`[MCP] ${req.method} /mcp -> authenticated`);
    next();
  }
  ...
  const publicServer = app.listen(publicPort, '0.0.0.0', () => {
    ...
    console.log('[MCP] Endpoint: /mcp (OAuth 2.1 + PKCE)');
