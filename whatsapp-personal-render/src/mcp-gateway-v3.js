...
export async function startMcpGateway({ publicPort, internalPort }) {
  ...
  function createWhatsappMcpServer() {
    const server = new McpServer({ name: 'meu-whatsapp', version: '1.2.0' });

    {
      const scopes = ['whatsapp.read'];
      const { schemes, meta } = authMeta(scopes);
      server.registerTool('whatsapp_status', {
        ...
        const denied = requireToolAuth(scopes);
  ...
  app.all('/mcp', parseMcpJson, (req, res) => {
    ...
    console.log(`[MCP] ${String(method)} -> ${payload ? 'authenticated' : 'discovery/unauthenticated'}`);
    requestAuth.run({ payload, metadataUrl, resource, issuer: base }, () => {
  ...
  const publicServer = app.listen(publicPort, '0.0.0.0', () => {
    ...
    console.log('[MCP] Endpoint: /mcp (public discovery + OAuth tools)');
