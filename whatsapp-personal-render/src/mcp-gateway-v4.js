...
export async function startMcpGateway({ publicPort, internalPort }) {
  ...
  function createWhatsappMcpServer() {
    const server = new McpServer({ name: 'meu-whatsapp', version: '1.3.0' });
  ...
  // Use the SDK's default Streamable HTTP behavior. JSON is returned for normal
  // requests and SSE is available when the protocol needs streaming.
  ...
  app.all('/mcp', parseMcpJson, (req, res) => {
    ...
    res.once('finish', () => {
      ...
      console.log(`[MCP] ${method} -> HTTP ${res.statusCode} ${contentType}`);
    });
  ...
  const publicServer = app.listen(publicPort, '0.0.0.0', () => {
    ...
    console.log('[MCP] Endpoint: /mcp (Streamable HTTP + tool-level OAuth)');
